...
This module implements a credential store for
the [keyring crate](https://crates.io/crates/keyring)
using the zbus-based Secret Service as its credential store.

You can specify this credential store as the default store for use
by keyring with the following code:
```
keyring::set_default_credential_builder(secret_service::default_credential_builder())
```
If you are planning to use this module with keyring, you will probably want to build
keyring without default features, in order to suppress the inclusion of its
built-in secret-store-based credential store.

Be sure to see the [docs of the keyring crate](https://docs.rs/keyring)
for info on a variety of known issues when using a secret-service-based
credential store.
...
If you are using `tokio` as your runtime, be careful:
...
mod tests {
    ...
    fn entry_new(service: &str, user: &str) -> Entry {
        let cred = SsCredential::new_with_target(None, service, user);
        match cred {
    ...
    fn delete_collection(name: &str) {
        use crate::{blocking::SecretService, EncryptionType};
